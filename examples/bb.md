...
5. For a read-only review: spawn a **child** on the **same folder**, or open another thread
   that can see the same folder. If it cannot, paste the packet from [beginner.md](beginner.md)
   into that chat (manual fallback). Use a **different model family**. The child does not `/close`.
6. Child returns findings to the head. Head decides. **Head** closes (you say wrap up in the parent).

Copyable head prompt:

```text
Read START.md. Goal: Draft a one-page fictional weekend walking tour of Lisbon.
Allowed file: drafts/lisbon-outline.md only. Forbidden: git, other paths, memory/ writes until I say wrap up.
Do not ask me for a different goal.
```

Copyable reviewer child prompt:

```text
Read-only. Same folder if you can see it; otherwise use the outline pasted below.
Do not edit files. Do not write memory/. Do not /close.
Different model family than the author. Return findings only.
```
